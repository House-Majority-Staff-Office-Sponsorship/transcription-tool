...
8. For testing purposes, the google drive configuration authentication must be obtained elsewhere, otherwise set the `GOOGLE_DRIVE_UPLOAD_ENABLED` field to false and the final transcripts will be saved to the local file system.  

9. Build the image (note that this image will be very large and may take several minutes to build especially the first time):
...
This project runs continously in a loop, and in each cycle:  
...
4. Stores the metadata in the database and the `.json` and `.txt` files in `/data/transcripts/<comitee_classification>` (for testing these files are uploaded to a google drive folder and deleted from the local filesystem).
